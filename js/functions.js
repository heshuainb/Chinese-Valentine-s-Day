...
function timeElapse(c) {
  ...
  var f = (now - c) / 1000; // 计算时间差（秒）
  var g = Math.floor(f / (3600 * 24)); // 计算天数
  f = f % (3600 * 24);
  var b = Math.floor(f / 3600); // 计算小时数
  if (b < 10) {
    b = "0" + b;
  }
  f = f % 3600;
  var d = Math.floor(f / 60); // 计算分钟数
  if (d < 10) {
    d = "0" + d;
  }
  f = f % 60;
  ...
  var a =
    ...
    f +
    "</span> seconds";
  $("#elapseClock").html(a); // 更新页面上的时间显示
}
function showMessages(){adjustWordsPosition();$("#messages").fadeIn(5000,function(){showLoveU()})}function adjustWordsPosition(){$("#words").css("position","absolute");$("#words").css("top",$("#garden").position().top+195);$("#words").css("left",$("#garden").position().left+70)}function adjustCodePosition(){$("#code").css("margin-top",($("#garden").height()-$("#code").height())/2)}function showLoveU(){$("#loveu").fadeIn(3000)};
